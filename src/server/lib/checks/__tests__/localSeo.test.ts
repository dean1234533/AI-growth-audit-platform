...
describe('local.gbp', () => {
  it('recognises a share.google link (previously missed entirely)', () => {
    ...
    expect(findCheck(runLocalSeoChecks(page), 'local.gbp').passed).toBe(true);
  });
  ...
  it('does not false-positive on an unrelated google.com link', () => {
    ...
    expect(findCheck(runLocalSeoChecks(page), 'local.gbp').passed).toBe(false);
  });
});
...
describe('local.locationPages', () => {
  it('recognises a /areas/<place> hub cluster, not just single-segment slugs', () => {
    ...
    const check = findCheck(runLocalSeoChecks(page), 'local.locationPages');
  ...
  it('still recognises the flat "personal-trainer-stratford" style cluster', () => {
    ...
    expect(findCheck(runLocalSeoChecks(page), 'local.locationPages').passed).toBe(true);
  });
});
...
describe('local.nap', () => {
  it('accepts a postcode district alone as a weaker but genuine signal', () => {
    ...
    const check = findCheck(runLocalSeoChecks(page), 'local.nap');
    expect(check.passed).toBe(true);
  ...
  it('still fails when there is no address signal at all', () => {
    ...
    expect(findCheck(runLocalSeoChecks(page), 'local.nap').passed).toBe(false);
  });
});
...
describe('local.reviews', () => {
  it('tolerates real-world phrasing like "9 verified Google Reviews"', () => {
    ...
    expect(findCheck(runLocalSeoChecks(page), 'local.reviews').passed).toBe(true);
  });
});
